...
function serveStaticFile(response, path, contentType, responseCode){
  fs.readFile(path, function(err, data){ //readFile 파일을 읽는데 사용되는 내장 메서드(비동기)
    if(err){
      response.writeHead(500,{'Content-Type':'text/plain'}); 
      /* writeHead (상태코드, 헤더 정보)
      500, 인터넷 서버 에러(내부적 서버 에러)
      'text/plain':읽을 수 있는 자료(그림X)의 문자열 데이터 */
...
const server=http.createServer((request, response)=>{ //createServer:서버 객체 생성
  const url=request.url.toLowerCase(); //toLowerCase:문자열을 소문자로 변환

  switch(url){ //조건문이 다양하거나 복잡할 때는 switch문 사용
    case '/':
